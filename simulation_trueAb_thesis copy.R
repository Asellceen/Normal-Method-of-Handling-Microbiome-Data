...
set.seed(1)
alpha_combo<- 7; alpha = alpha_combo
...
set.seed(2)
theta_combo <- rnorm(62, mean = c(rep(1, K1_combo), rep(3, 62- K1_combo)), sd=1); theta = theta_combo
...
true_abundance<- function(alpha, theta, sigma, rho, D_phylo, n, k, Ni){
  ...
  for (j in 1:k){
    delta_param[j]<- rbinom(1, 1, p_delta1[1,j])
  }
