...
otu_list_tree2<- list()
otu_zero_prop_tree2<- list()
for (i in 1:100){
  set.seed(i)
  otu_list_tree2[[i]]<- otu_generator(n= 98, k= 62, true_abundance = true_abundance_combo2, ni= ni_combo)$otu
  otu_zero_prop_tree2[[i]]<- otu_generator(n= 98, k= 62, true_abundance = true_abundance_combo2, ni= ni_combo)$prop_zeros
}
save(otu_list_tree2, file= "otu_counts_tree2.RData")
save(otu_zero_prop_tree2, file= "otu_zero_prop_tree2.RData")
