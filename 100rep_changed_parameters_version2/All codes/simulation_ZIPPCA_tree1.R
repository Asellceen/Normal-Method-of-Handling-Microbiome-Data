...
imputed_ZIPPCA_tree1<- list()
prop_zeros_noColZeros_ZIPPCA_tree1<- list()
column_remove_ZIPPCA_tree1<- list()
time_ZIPPCA_tree1<- list()
for (i in 1:500){
  temp_value<- fitting_ZIPPCA(otu_list_tree1[[i]])
  imputed_ZIPPCA_tree1[[i]]<- temp_value$imputed_value_ZIPPCA
  prop_zeros_noColZeros_ZIPPCA_tree1[[i]]<- temp_value$prop_zeros_noColZeros
  column_remove_ZIPPCA_tree1[[i]]<- temp_value$column_remove
  time_ZIPPCA_tree1[[i]]<- temp_value$time_ZIPPCA
}
save(imputed_ZIPPCA_tree1, file= "imputed_ZIPPCA_tree1.RData")
save(prop_zeros_noColZeros_ZIPPCA_tree1, file= "prop_zeros_noColZeros_ZIPPCA_tree1.RData")
save(column_remove_ZIPPCA_tree1, file= "column_remove_ZIPPCA_tree1.RData")
save(time_ZIPPCA_tree1, file= "time_ZIPPCA_tree1.RData")
